...
class Lot(object):
    ...
    def __eq__(self, other):
        return (
            ...
            and self.buy_date == other.buy_date
            and self.adjusted_buy_date == other.adjusted_buy_date
            and self.basis == other.basis
            and self.adjusted_basis == other.adjusted_basis
            and self.sell_date == other.sell_date
            and self.proceeds == other.proceeds
            and self.adjustment_code == other.adjustment_code
            and self.adjustment == other.adjustment
            and self.form_position == other.form_position
            and self.buy_lot == other.buy_lot
            and self.replacement_for == other.replacement_for
            and self.is_replacement == other.is_replacement
            and self.loss_processed == other.loss_processed
        )
...
class Lots(object):
    ...
    def _terminaltables_str(
        ...
        for lot in lots:
            ...
            if classification:
                ...
                str_data = list(
                    map(lambda x: Lots._color_string(color, x), str_data)
                )
            else:
                str_data.append("")
    ...
    def _simple_str(
        ...
        lot_strings = []
        lot_strings.append(
            " ".join([self.SHORT_HEADERS[field] for field in Lot.FIELD_NAMES])
        )
        for lot in lots:
            classification = Lots._classify_lot(
                lot,
    ...
    def create_from_csv_data(data):
        ...
        for row in reader:
            ...
            row["replacement_for"] = convert_to_string_list(
                row["replacement_for"]
            )
